<?php
  ...
  // $officer = $POST_[$_SESSION['id']];
       ...
       //check if employee ot exits already  
        $check = sqlsrv_query($conn, "SELECT * FROM bz WHERE duplicate='$duplicate'");
        ...
        if($checkrows>0)
        {
             ...
             echo"</script>";
           include_once 'bzclerkaddot.php';
        }
      
   else
   {
        //insert results from the form input	
	$sql = "INSERT INTO bz (duplicate,social,name,timefrom,timeto,hours,firm,serviceperform,salary,officerpost,costcentercode,program,activitycode,otrate,wklyhrs,fname,lname,lineitemcode,amount,dateentered, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $params = array($duplicate, $empssn, $name,$datetimefrom, $dtimeto, $hours, $firm, $serviceperform, $salary, $officerpost, $costcentercode, $program, $activitycode, $otrate, $wklyhrs, $fname, $lname, $licode, $amount, $dateentered, $location);	
