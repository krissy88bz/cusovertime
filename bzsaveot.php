  ...
  // $officer = $POST_[$_SESSION['id']];
       ...
       //check if employee ot exits already  
        ...
        if($checkrows>0)
        {
             ...
             echo"</script>";
           include_once 'bzaddot.php';
        }
      
   else
   {
        //insert results from the form input	
	$sql = "INSERT INTO bz (duplicate,social,name,timefrom,timeto,hours,firm,serviceperform,salary,officerpost,costcentercode,program,activitycode,otrate,wklyhrs,fname,lname,lineitemcode,amount,dateentered) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
